...
//			@ApiResponse(code = 500, message = "Erro interno no servidor") })
	 ...
	 * 
	 */
//	@ApiOperation(value = "Lista todos os registros de Relatórios Finais cadastrados no sistema")
//	@ApiResponses(value = { @ApiResponse(code = 200, message = "Sucesso", response = RelatorioFinal.class),
//			@ApiResponse(code = 201, message = "Criado"), @ApiResponse(code = 204, message = "Sem conteúdo"),
//			@ApiResponse(code = 401, message = "Sem autorização"), @ApiResponse(code = 403, message = "Proibido"),
//			@ApiResponse(code = 404, message = "Não encontrado"),
...
//			@ApiResponse(code = 500, message = "Erro interno no servidor") })
	@PostMapping()
	public ResponseEntity<RelatorioFinal> salvar(@RequestBody RelatoriosAlunoWrapper relatoriosAlunoWrapper, HttpServletResponse response) throws BusinessException {
		return relatorioFinalService.salvar(relatoriosAlunoWrapper, response);
	}
}
